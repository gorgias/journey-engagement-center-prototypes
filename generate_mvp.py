...
with sync_playwright() as p:
    ...
    removed = page.evaluate('''() => {
        ...
        document.querySelectorAll('.navItem').forEach(el => {
            ...
            if (['opportunities','intelligence','loyalty'].includes(pg)) {
                removed.push('nav:' + pg);
                el.remove();
            }
        });
        
        // Remove Overview benchmark
        document.querySelectorAll('#page-overview .card').forEach(card => {
            if (card.textContent.includes('How you compare')) {
                removed.push('overview-benchmark');
                card.remove();
            }
        });
        ...
        ['page-opportunities','page-intelligence','page-loyalty'].forEach(id => {
            ...
            if (el) { removed.push(id); el.remove(); }
        });
        
        // Remove Custom engagement tab and panel
        ...
        if (acePanel) {
            acePanel.classList.remove('tabPanel');
            acePanel.removeAttribute('data-panel-for');
        }
        document.querySelectorAll('#page-engagements .tabList').forEach(el => {
            if (el.children.length === 0) el.remove();
        });
        
        // Remove New engagement button
        document.querySelectorAll('button[data-panel="new"]').forEach(el => {
            removed.push('btn:new-engagement'); el.remove();
        });
        
        // Update engagements subtitle
        ...
        // Remove Campaigns AI opportunities
        document.querySelectorAll('#page-campaigns .col.gap-xs').forEach(col => {
            const h2 = col.querySelector('h2');
            if (h2 && h2.textContent.includes('AI opportunities')) {
                removed.push('campaigns-ai-opportunities'); col.remove();
            }
        });
        
        // Remove opportunity sections
        ['guardrailOpportunities','lifecycleOpportunities','loyaltyOpportunities'].forEach(id => {
            const el = document.getElementById(id);
            if (el) { removed.push(id); el.remove(); }
        });
        
        // Remove askFab/askPanel
        ...
        // Remove Manual acceptance mode card
        ...
        // Channels: keep only SMS in list
        const chList = document.getElementById('chList');
        if (chList) {
            Array.from(chList.children).slice(1).forEach(el => el.remove());
            removed.push('channels-non-sms');
        }
...
# Now do safe string replacements on the serialized HTML
# Replace ACE_DOMAINS with flat engagements
...
if old_domains in html:
    ...
    print('WARNING: ACE_DOMAINS not found in serialized HTML')
...
if old_helpers in html:
    ...
    print('WARNING: helper functions not found')
...
# Replace renderAce - this is the tricky part. We'll do it via DOM manipulation instead.
# Actually, since we already serialized, let's just replace the function body by finding it.
# Better: inject a script at the end that overrides renderAce.
override_script = '''
<script>
(function(){
  // Override renderAce for flat engagements
  ...
  // Override click handlers for flat engagements
  const oldClick = document.onclick;
  document.addEventListener('click', function(e){
    const domToggle=e.target.closest("[data-eng]");
    if(domToggle){
      const i=+domToggle.dataset.eng;
...
html = html.replace('</body>', override_script + '\n</body>')
